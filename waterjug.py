...
def a_star_algorithm(jug1_capacity, jug2_capacity, goal):
...
def water_jug_solver(jug1_capacity, jug2_capacity, goal):
    path, cost = a_star_algorithm(jug1_capacity, jug2_capacity, goal)
    if path:
        print(f"Solution found in {cost} steps with the goal in Jug:")
        for state in path:
            print(state)
    else:
        print("No solution found.")
...
# target_jug = int(input("Which jug should contain the goal amount? (1 or 2): "))  # User input for target jug
...
water_jug_solver(jug1_capacity, jug2_capacity, goal)
